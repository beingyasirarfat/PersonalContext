...
function getTimeInput(callback) {
  const [picker, input] = ["datetimePicker", "datetimePickerInput"].map((id) =>
  ...
  input.addEventListener("focusout", () => {
    ...
    chrome.storage.local.set({
      [callback === elapsedTime ? "initialTime" : "targetTime"]:
...
(function () {
  ...
  audio.src = `/assets/clockSounds/${
    ["chic", "click", "slash", "tick", "ticktac", "ticktock"][
  ...
  chrome.storage.local.get(
    ...
    ({ clockTicking, initialTime, targetTime }) => {
      const initTime = isNaN(Date.parse(initialTime))
        ? new Date("May 08, 1997 00:00:00")
        : new Date(initialTime);
      const tgtTime = isNaN(Date.parse(targetTime))
