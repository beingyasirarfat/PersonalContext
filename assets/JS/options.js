...
SaveSettings.onclick = function (e) {
  ...
  chrome.storage.local.set(
    ...
    () =>
      ServerStorage.checked
        ? flash('Database address set to "' + uri + '"')
        : flash("success")
  );
};
...
ResetSettings.onclick = function (e) {
  e.preventDefault();
  if (
    confirm("You are going to lose the settings and data stored. Are you sure?")
  ) {
    // clear all the data
...
ClearHistory.onclick = function (e) {
  e.preventDefault();
  if (
    confirm("You are going to clear all the words saved locally. Continue?")
  ) {
    chrome.storage.local.set({ Words: [], Notes: [], Facts: [] }, () =>
      flash("success")
    );
  }
};
...
(function () {
  chrome.storage.local.get(
    { Server: false, ServerURL: "", SoundFeedback: true },
    function (data) {
      if (data.Server) {
        CacheStorage.checked = false;
